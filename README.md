...
- Fits a Weibull distribution to GAGEBRTH, defines 1st and 99th percentile cutoffs, and filters samples outside this range. It summarizes pre-term vs. normal birth counts per categorical class, removes low-count classes (<20 total), and drops categorical variables entirely if only one class would remain. It reports which categorical variables are retained and PTB #s(in CategoricalVariablesToKeepTable.tsv), flags binary variables for feature selection, and outputs Weibull parameters, cutoffs, and plots. It finds haplogroups that appear in at least 2 sites(unless single site analysis), have ≥ 20 total samples and ≥ 3 PTB cases, keeps those unchanged, and relabels all other haplogroups as Other_population.Finally, it saves the filtered metadata (Metadata.Weibull.tsv), writes IDs2.txt for downstream nDNA selection, and plots both filtered/unfiltered distributions and all continuous feature relationships with PTB and GA (outputs to plotsAll). Makes a report (out.txt) which will indicate which variables to keep/exclude in future modeling. 
...
- If you have site as a categorical variable, you cannot use population. 
