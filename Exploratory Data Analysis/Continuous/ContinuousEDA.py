...
haplogroups= pd.get_dummies(df[sys.argv[2]], drop_first=False).columns.to_list()
df = pd.concat([df[continuous_vars + ['PTB', 'GAGEBRTH']], pd.get_dummies(df[sys.argv[2]], drop_first=False).astype(int)],axis=1)
