...
formula_PTB = 'PTB ~ ' + ' + '.join(features)

formula_GA = 'GAGEBRTH ~ ' + ' + '.join(features)
...
glm_fit_ga = smf.glm("GAGEBRTH ~ population + PC1 + PC2 + DIABETES + PW_AGE + BMI", data=df).fit()
summary_df_ga = detailed_model_summary(glm_fit_ga, df, 'population')
print(summary_df_ga)
# Fit models for PTB
print(f"Population and CoVar -> PTB, Hapologroup Ref={ref}")
glm_fit_ptb = smf.glm("PTB ~ population + PC1 + PC2 + DIABETES + PW_AGE + BMI", family=sm.families.Binomial(), data=df).fit()
...
glm_fit_ga = smf.glm("GAGEBRTH ~ site + PC1 + PC2 + DIABETES + PW_AGE + BMI", data=df).fit()
summary_df_ga = detailed_model_summary(glm_fit_ga, df, 'site')
print(summary_df_ga)
# Fit models for PTB
print(f"Site and CoVar -> PTB, Hapologroup Ref={ref}")
glm_fit_ptb = smf.glm("PTB ~ site + PC1 + PC2 + DIABETES + PW_AGE + BMI", family=sm.families.Binomial(), data=df).fit()
